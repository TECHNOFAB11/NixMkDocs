...
For each documentation site configured using the [`doc`](configuration.md#doc) or [`docs`](configuration.md#docs) options, NixMkDocs generates two packages in your system's `legacyPackages` (or equivalent, depending on your `flake-parts` setup). The name of these packages is derived from the name given to the documentation entry (e.g., `"default"`, `"user-guide"`, `"api-docs"`).

Let's assume your documentation site is named `<name>` (e.g., `default` if using the `doc` option, or the key used in the `docs` attrset).
